...
#### 5.3.1 自动模式（Auto reclaude）

如果你用的是 [reclaude](https://reclaude.ai) 拼车账号，可以让 daemon **完全自动**接力 5h 窗口，不需要你每天手动加时间点。

**工作原理**：daemon 每 10 分钟拉一次 reclaude 的 `carpool-quota` 接口，读取当前窗口的 `resets_at_ms`（窗口关闭时间），自动在列表里维持一个时间为 `resets_at_ms + 30 秒` 的触发点。当前窗口一关闭，那个点立刻触发，新窗口被激活。整个过程无需人工介入。

**怎么开启**：

1. 在配置卡里把 **调度模式** 切到 **Auto (reclaude)**
2. 输入你的 reclaude 邮箱和密码，点 **登录并启用**
3. 把右上的主开关切到 **已启用**
4. 等几秒，列表里会自动出现一个 🔁 标记的"自动点"

**自动模式下界面行为变化**：

- 触发时间点卡的"**添加触发点**"表单会**隐藏**（daemon 自己管，不需要你加）
- 列表里 daemon 维护的点带 🔁 徽章，普通 manual 点不带
- 配置卡顶部会显示当前剩余额度（USD 进度条）+ 窗口剩余时间
- 你手动删一个 🔁 点也行，下一轮 poll（最多 10 分钟）会自动补回来

**怎么解绑**：在配置卡里点 **解绑**。daemon 会清掉本地存的 cookie + 密码，模式回到 manual。已经在列表里的 🔁 点会留着自然触发，只是不再续接新的。

**安全提醒**：登录后 daemon 把 cookie 和**密码本身**存到 `~/claude-quota-warmer/data/secrets.json`（权限 chmod 0600，只有你能读）。这是个工程权衡——存了密码，cookie 过期才能自动用密码重新登录、保持长期运行；不存的话每次 cookie 过期都要你手动重输。如果你介意明文密码落盘，就不要用 auto 模式，继续用 manual 模式即可。
...
### Q: 听说 v0.2.0 加了自动模式，跟手动模式什么区别？

**手动模式（默认）**：你自己决定每个时间点，daemon 只到点执行。完全本地化、不联外网（除了 reclaude/claude 命令本身）、不存任何账号密码。

**自动模式**：你绑定 reclaude 拼车账号后，daemon 每 10 分钟轮询 reclaude 的额度接口，自动在你的窗口结束时排下一次触发，长期挂机不用管。但代价是：①只能跟 reclaude 拼车账号配合用 ②账号密码会以明文（0600 权限）存在本地 `data/secrets.json` 里，daemon 用它来自动刷新过期 cookie。

如果你介意明文密码落盘，用手动模式即可，功能上完全够用，只是要自己加时间点。

### Q: 自动模式的密码存哪里？安全吗？

存在 `~/claude-quota-warmer/data/secrets.json`，文件权限 `chmod 0600`（只有你自己能读）。原子写（先写 tmp 再 rename）。不会进 git（已被 `.gitignore` 覆盖）。不会出现在任何 API 返回里（响应里只有 `has_password: true/false` 这个布尔）。

不安全的场景：①你的整台电脑被攻破（这种情况你的 SSH key、浏览器 cookie 也都泄露了，本工具不是特别的弱点）②你把 `data/` 整个备份上传到了不受控的地方。注意一下这两点就好。
